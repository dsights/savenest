...
## 8. Comparison-as-a-Service (CaaS) Integration
*   **Module:** `lib/features/comparison/`
*   **White-label Solution:** Partnered with CIMET/AccuRate for self-service comparison.
*   **Implementation:** 
    *   `lib/features/comparison/widgets/partner_webview.dart` handles the embedded partner tool.
    *   `ComparisonScreen` contains entry points (AppBar icon and CTA button).
*   **Pending Action:** Update the placeholder URL (`https://flutter.dev`) in `ComparisonScreen` with the CIMET affiliate link once available.
