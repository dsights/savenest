...
import 'package:savenest/services/api_service.dart';
...
class _RegistrationScreenState extends ConsumerState<RegistrationScreen> {
  ...
  Future<void> _checkBasiqStatus() async {
    try {
      final response = await http.get(Uri.parse('${ApiService.backendBaseUrl}/api/status'));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          _isBasiqEnabled = data['basiq_enabled'];
        });
      }
    } catch (e) {
      // Basiq is considered disabled if the status check fails
      setState(() {
        _isBasiqEnabled = false;
      });
    }
  }
  ...
  Future<void> _connectProvider() async {
    ...
    try {
      ...
      final userResponse = await http.post(
        Uri.parse('${ApiService.backendBaseUrl}/users'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'email': _emailController.text}),
      );

      if (userResponse.statusCode != 200) {
        throw Exception('Failed to create user: ${userResponse.body}');
      }
      ...
      final connectionResponse = await http.post(
        Uri.parse('${ApiService.backendBaseUrl}/users/$userId/connections'),
      );

      if (connectionResponse.statusCode != 200) {
        throw Exception('Failed to create connection: ${connectionResponse.body}');
      }

      final connectionData = jsonDecode(connectionResponse.body);
      final connectionUrl = connectionData['data']['links']['public'];
      
      // 3. Launch URL
      if (await canLaunchUrl(Uri.parse(connectionUrl))) {
        await launchUrl(Uri.parse(connectionUrl), webOnlyWindowName: '_blank');
      } else {
        throw 'Could not launch $connectionUrl';
      }

    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error connecting provider: $e')),
        );
      }
    }
    finally {
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                                                    ...
                                                    if (_isBasiqEnabled) ...[
                                                      Text(
                                                        'Connect your provider to automatically fetch your bill.',
                                                        style: TextStyle(color: AppTheme.deepNavy.withOpacity(0.7)),
                                                      ),
                                                      const SizedBox(height: 16),
                                                      ElevatedButton(
                                                        onPressed: _isSubmitting ? null : _connectProvider,
                                                        child: const Text('Connect with your Provider'),
                                                      ),
                                                      const SizedBox(height: 16),
                                                      Center(child: Text("OR", style: TextStyle(color: AppTheme.deepNavy.withOpacity(0.7)))),
                                                      const SizedBox(height: 16),
                                                    ],
                                                    Text(
